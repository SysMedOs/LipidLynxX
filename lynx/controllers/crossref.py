...
async def get_cross_ref(lipid_name: str = "PC(16:0/18:2(9Z,12Z))", export_url: bool = False) -> dict:
    ...
    linked_ids = {}
    swiss_cross_refs = ["ChEBI", "HMDB", "LipidMaps", "SwissLipids", "Rhea", "UniProtKB"]
...
async def get_swiss_ref(swisslipids_id, ref_db, export_url: bool = False) -> Union[dict, list]:
    ...
    ref_dbs = ["ChEBI", "HMDB", "LipidMaps", "SwissLipids", "Rhea", "UniProtKB"]
    ...
    if swisslipids_id and ref_db in ref_dbs:
        if ref_db.lower() == "swisslipids":
            if export_url and swisslipids_id not in cross_ref_id_urls:
                cross_ref_id_urls[swisslipids_id] = await get_ref_link(
    ...
    if export_url:
        cross_ref_id_urls = {k: cross_ref_id_urls[k] for k in natsort.natsorted(cross_ref_id_urls.keys())}
...
async def get_ref_link(ref_id: str, ref_db: str) -> str:
    ...
    ref_db_urls = {
        ...
        "SwissLipids": "https://www.swisslipids.org/#/entity/LIPID_ID/",
