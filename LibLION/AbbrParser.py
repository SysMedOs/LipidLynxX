...
class AbbrParser:
    ...
    def __init__(self, cfg):
        ...
        self.fa_lipostar_rgx = re.compile(r'(?P<LINK>FA|O-|P-)?(?P<NUM_C>\d{1,2})(:)(?P<NUM_DB>\d)'
                                          r'(?P<MOD_INFO>\(.*\))?')
        self.fa_lipidmaps_rgx = re.compile(r'(?P<LINK>FA|O-|P-)?(?P<NUM_C>\d{1,2})(:)(?P<NUM_DB>\d)'
        ...
        if fa_match:
            ...
            if fa_info_dct['MOD_INFO']:
                ...
                for mod_type in mod_sum_info_dct:
                    mod_info_str = '{' + f"{','.join(natsorted(mod_sum_info_dct[mod_type]))}" + '}'
                    mod_sum_info_lst.append(f"{len(mod_sum_info_dct[mod_type])}{mod_type}{mod_info_str}")
