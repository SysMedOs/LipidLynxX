            ...
            if s_name != '_version':
                ...
                if f.endswith('.schema.json'):
                    if os.path.isfile(f):
                        ...
                        raise IOError(f'Cannot find schema.json file and json example: {f}')
            else:
                logger.info(f'API schema version: {lynx_schema[s_name]}')

        if not self.schema_test_dct:
            raise IOError(f'Cannot find schema.json file and json example in settings: {json.dumps(lynx_schema)}')
